...
impl<const N: usize> DeviceContext<N> {
    ...
    pub fn init_from_preloaded_crs<#[cfg(feature = "allocator")] A: HostAllocator>(
        domain_size: usize,
        #[cfg(feature = "allocator")] crs: Crs<CompactBn256, CrsForMonomialForm, A>,
        #[cfg(not(feature = "allocator"))] crs: Crs<CompactBn256, CrsForMonomialForm>,
    ) -> CudaResult<Self> {
    ...
    fn init_msm_on_static_memory<#[cfg(feature = "allocator")] A: HostAllocator>(
        domain_size: usize,
        #[cfg(feature = "allocator")] crs: Option<Crs<CompactBn256, CrsForMonomialForm, A>>,
        #[cfg(not(feature = "allocator"))] crs: Option<Crs<CompactBn256, CrsForMonomialForm>>,
    ) -> CudaResult<()> {
    ...
    unsafe fn init_msm_on_pool<#[cfg(feature = "allocator")] A: HostAllocator>(
        domain_size: usize,
    ) -> CudaResult<()> {
    ...
    fn inner_init_msm<#[cfg(feature = "allocator")] A: HostAllocator>(
        domain_size: usize,
        #[cfg(feature = "allocator")] crs: Option<Crs<CompactBn256, CrsForMonomialForm, A>>,
        #[cfg(not(feature = "allocator"))] crs: Option<Crs<CompactBn256, CrsForMonomialForm>>,
        ...
        let crs = match crs {
            ...
            None => {
                ...
                let crs_file = std::fs::File::open(&crs_file_path)
                    .expect(&format!("crs file at {}", crs_file_path));

                #[cfg(feature = "allocator")]
                let crs = init_compact_crs::<_, A>(crs_file).unwrap();
                #[cfg(not(feature = "allocator"))]
                let crs = init_compact_crs(crs_file).unwrap();
...
#[cfg(feature = "allocator")]
pub fn init_compact_crs<R: std::io::Read, A: Allocator + Default>(
    src: R,
) -> std::io::Result<Crs<CompactBn256, CrsForMonomialForm, A>> {
    let num_g1_bases = MAX_COMBINED_DEGREE_FACTOR << fflonk::L1_VERIFIER_DOMAIN_SIZE_LOG;
    let mut g1_bases = Vec::with_capacity_in(num_g1_bases, A::default());
    let mut g2_bases = Vec::with_capacity_in(2, A::default());
    unsafe {
        g1_bases.set_len(num_g1_bases);
        g2_bases.set_len(2);
    }

    init_compact_crs_into(src, &mut g1_bases, &mut g2_bases)?;

    Ok(Crs::new_in(g1_bases, g2_bases))
}

#[cfg(not(feature = "allocator"))]
pub fn init_compact_crs<R: std::io::Read>(
    src: R,
) -> std::io::Result<Crs<CompactBn256, CrsForMonomialForm>> {
    let num_g1_bases = MAX_COMBINED_DEGREE_FACTOR << fflonk::L1_VERIFIER_DOMAIN_SIZE_LOG;
    let mut g1_bases = Vec::with_capacity(num_g1_bases);
...
pub fn init_compact_crs_into<R: std::io::Read>(
    mut src: R,
    g1_bases: &mut [CompactG1Affine],
    g2_bases: &mut [CompactG2Affine],
) -> std::io::Result<()> {
    use byteorder::{BigEndian, ReadBytesExt};
    let num_g1_bases = src.read_u64::<BigEndian>()? as usize;
    assert_eq!(g1_bases.len(), num_g1_bases);
    unsafe {
        let buf = std::slice::from_raw_parts_mut(
            g1_bases.as_mut_ptr() as *mut u8,
            num_g1_bases * std::mem::size_of::<bellman::compact_bn256::G1Affine>(),
        );
        src.read_exact(buf)?;
    }

    g2_bases.copy_from_slice(&hardcoded_g2_bases::<bellman::compact_bn256::Bn256>());
