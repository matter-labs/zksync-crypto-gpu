    ...
    F: PrimeField,
{
    pub fn allocate_on(device: &Device, domain_size: usize) -> CudaResult<Self> {
        match device {
            Device::A100_40(_) | Device::A100_80(_) | Device::L4(_) => {
...
#[derive(Default)]
pub(crate) struct PinnedAllocator;
...
pub struct CombinedMonomialHostStorage<F: PrimeField> {
    pub(crate) combined_monomials: [Vec<F, PinnedAllocator>; 3],
    ...
    F: PrimeField,
{
    fn allocate_on(domain_size: usize) -> CudaResult<Self> {
        ...
        let combined_monomials = std::array::from_fn(|_| {
            let mut buf = Vec::with_capacity_in(common_combined_degree, PinnedAllocator::default());
