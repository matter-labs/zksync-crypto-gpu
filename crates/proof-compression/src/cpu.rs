...
use bellman::{CurveAffine, Engine, Field, PrimeField};
use byteorder::{BigEndian, ReadBytesExt};
use circuit_definitions::circuit_definitions::aux_layer::compression_modes::{
    CompressionTranscriptForWrapper, CompressionTreeHasherForWrapper,
};
...
use franklin_crypto::boojum::cs::implementations::hints::{
    DenseVariablesCopyHint, DenseWitnessCopyHint,
};
use franklin_crypto::boojum::cs::implementations::polynomial_storage::{
...
use franklin_crypto::boojum::{
    ...
    cs::{
        ...
        implementations::{
            ...
            setup::FinalizationHintsForProver, verifier::Verifier,
        },
    },
...
impl CompressionInput {
    pub fn into_compression_circuit(self) -> ZkSyncCompressionLayerCircuit {
        match self {
            ...
            CompressionInput::CompressionLayer(proof, vk, compression_mode) => {
    ...
    pub fn into_compression_wrapper_circuit(self) -> ZkSyncCompressionForWrapperCircuit {
        match self {
            ...
            CompressionInput::CompressionWrapperLayer(proof, vk, compression_mode) => {
    ...
    */
    ...
    for step_idx in 0..num_compression_steps {
        ...
        let proof_file_path = format!(
            "./data/compression_schedule/{compression_schedule_name}/compression_{}_proof.json",
            compression_mode as u8
        );
        let proof_file_path = std::path::Path::new(&proof_file_path);
        let vk_file_path = format!(
            "./data/compression_schedule/{compression_schedule_name}/compression_{}_vk.json",
            compression_mode as u8
        );
        ...
        save_compression_proof_and_vk_into_file(
            &proof,
            &vk,
            compression_schedule_name,
            circuit_type as u8,
        );
        if step_idx + 1 == num_compression_steps {
            input = CompressionInput::CompressionWrapperLayer(
...
pub fn compress_proof_with_precomputations(
    proof: ZkSyncRecursionProof,
    vk: ZkSyncRecursionVerificationKey,
    schedule: CompressionSchedule,
    ...
    */
    ...
    for step_idx in 0..num_compression_steps {
        ...
        let compression_circuit = input.into_compression_circuit();
        assert_eq!(
            compression_circuit.numeric_circuit_type(),
            compression_mode as u8
        );
        println!(
            "Proving compression {compression_schedule_name}/{}",
        ...
        let (proof, vk) = match compression_circuit {
            ZkSyncCompressionLayerCircuit::CompressionMode1Circuit(inner) => {
                prove_compression_circuit_with_precomputations(
                    inner,
                    proof_config,
                    &worker,
    ...
    if proof_file_path.exists() && vk_file_path.exists() {
        ...
        assert_eq!(
            compression_wrapper_circuit.numeric_circuit_type(),
            last_compression_wrapping_mode as u8
        );
...
pub fn precompute_and_save_setup_for_compression_steps(
    vk: ZkSyncRecursionVerificationKey,
    schedule: CompressionSchedule,
    output_blob_path: &std::path::Path,
) {
    let worker = BoojumWorker::new();
    ...
    for step_idx in 0..num_compression_steps {
        ...
        let compression_circuit = input.into_compression_circuit();
        println!(
            "Generating precomputed setup for compression layer {compression_schedule_name}/{}",
            compression_mode as u8
        );
        let (setup_base, setup, setup_tree, vars_hint, wits_hint, vk) =
            inner_precompute_setup_for_compression_layer(compression_circuit, &worker);
        ...
        if step_idx + 1 == num_compression_steps {
            input = CompressionInput::CompressionWrapperLayer(
                None,
                vk,
                CompressionMode::from_compression_mode(last_compression_wrapping_mode as u8),
            );
        } else {
            input = CompressionInput::CompressionLayer(
                None,
                vk,
                CompressionMode::from_compression_mode(step_idx as u8 + 1),
            );
        }
    }
    ...
    let (setup_base, setup, setup_tree, vars_hint, wits_hint, vk) =
        inner_precompute_setup_for_compression_wrapper(compression_wrapper_circuit, &worker);
    save_precomputations_into_file(
        &setup_base,
        &setup,
        &setup_tree,
    ...
    H::Output: serde::de::DeserializeOwned,
{
    ...
    let vk_file_path = format!(
        "{}/setup_compression_{}.setup",
        blob_path, compression_mode as u8
    );
...
{
    let setup_base_file_path = format!(
        "{}/setup_base_{}.setup",
        output_blob_path, compression_mode as u8
    );
    ...
    let setup_tree_file_path = format!(
        "{}/setup_tree_{}.setup",
        output_blob_path, compression_mode as u8
    );
    ...
    let vars_hint_file_path = format!(
        "{}/vars_hint{}.setup",
        output_blob_path, compression_mode as u8
    );
    ...
    let wits_hint_file_path = format!(
        "{}/wits_hint{}.setup",
        output_blob_path, compression_mode as u8
    );
...
pub fn prove_compression_circuit_with_precomputations<CF: ProofCompressionFunction>(
    ...
    BoojumProof<F, CompressionTreeHasher, EXT>,
    BoojumVK<GoldilocksField, CompressionTreeHasher>,
) {
    let (setup_base, setup, setup_tree, vars_hint, wits_hint, vk) =
        load_precomputations_from_file::<CompressionTreeHasher>(blob_path, compression_mode);
...
pub fn prove_compression_wrapper_circuit_with_precomputations<CF: ProofCompressionFunction>(
    ...
    BoojumProof<F, CompressionTreeHasherForWrapper, EXT>,
    BoojumVK<GoldilocksField, CompressionTreeHasherForWrapper>,
) {
    let (setup_base, setup, setup_tree, vars_hint, wits_hint, vk) = load_precomputations_from_file::<
